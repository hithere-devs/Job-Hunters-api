...
if(!['openrouter','vertex'].includes(provider))throw new Error('OpenClaw supports MODEL_PROVIDER=openrouter or vertex');
...
const modelRef=provider==='vertex'?`google-vertex/${vertexModel}`:`openrouter/${model}`;
...
for(let i=1;i<=10;i++){
 ...
 const modelPlugin=provider==='vertex'?'google':'openrouter';
 const modelProvider=provider==='vertex'?{'google-vertex':{apiKey:'gcp-vertex-credentials'}}:{openrouter:{apiKey:'${OPENROUTER_API_KEY}'}};
 const config={gateway:{mode:'local',port,bind:'loopback',auth:{mode:'token',token},controlUi:{enabled:false}},agents:{defaults:{workspace:`${state}/workspace`,model:{primary:modelRef,fallbacks:[]},models:{[modelRef]:{params:{maxTokens:outputCap}}},thinkingDefault:'low',timeoutSeconds:300,maxConcurrent:1,heartbeat:{every:'0m'},modelPolicy:{allow:[modelRef]}}},models:{mode:'merge',providers:modelProvider},browser:{enabled:true,evaluateEnabled:false,attachOnly:true,defaultProfile:'tenant',profiles:{tenant:{cdpUrl:`http://127.0.0.1:${9200+i}`,attachOnly:true}},tabCleanup:{enabled:false}},tools:{profile:'full',allow:['browser'],loopDetection:{enabled:true},web:{search:{enabled:false},fetch:{enabled:false}}},plugins:{slots:{memory:'none'},allow:['browser','huntly-application-guard',modelPlugin],load:{paths:['/opt/huntly/openclaw-guard']},entries:{'memory-core':{enabled:false},perplexity:{enabled:false},[modelPlugin]:{enabled:true},browser:{enabled:true},'huntly-application-guard':{enabled:true}}}};
