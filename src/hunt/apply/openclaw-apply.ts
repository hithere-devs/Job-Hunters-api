...
export function buildOpenClawApplyPrompt(input: OpenClawApplyInput): string {
  ...
  return [
    'You are filling one job application in the already-open tenant Chrome. The deterministic ladder already filled known fields. Continue from the current page, do not restart or open another profile. Preserve correctly filled answers. If the resume is already attached with its filename, do not upload it again.',
    ...
    'For boolean saved answers, true means Yes and false means No. A failed prior DOM fill does not invalidate a saved user answer. Do not ask again for facts in this dossier. Resolve wording variations and format values to match the offered options. Use current-attempt profile_ai answers exactly; Huntly already checked their evidence and field options. Record them under assumptions.',
    ...
    'For ordinary professional free text, write a short answer grounded in the supplied resume and profile. Connect and paraphrase those facts for this role, but do not invent employers, dates, achievements, credentials, URLs, or personal events. Choose privacy-safe defaults for optional communication and demographic questions. Record every inferred or default answer under assumptions. Optional unknown facts stay blank. Required facts with no support must be reported as blocked.',
    ...
    'For resume upload use browser action:"upload", inputRef:"eN" for the actual file input, and paths:[the supplied resumePath]. Do not use ref, element, a selector, or a native file chooser for upload. A file input can appear as a button in the accessibility snapshot.',
