...
assert.match(settings,/^HUNTLY_BROWSER_PROFILE=(?:"extension-test"|'extension-test'|extension-test)$/m,'optional extension guard must be explicitly enabled')
...
try{
 ...
 <label for="fixture-resume">Fixture resume</label><input id="fixture-resume" type="file" accept="application/pdf" required><button type="submit">Submit application</button></form>
 ...
 const actualModel=transcript?database.prepare("SELECT json_extract(event_json,'$.message.provider') AS provider,json_extract(event_json,'$.message.model') AS model FROM transcript_events WHERE session_id=? AND json_extract(event_json,'$.message.role')='assistant' ORDER BY seq DESC LIMIT 1").get(transcript.id):null
