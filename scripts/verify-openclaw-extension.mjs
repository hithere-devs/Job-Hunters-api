...
try{
 ...
 const denied=transcript?database.prepare("SELECT COUNT(*) AS n FROM transcript_events WHERE session_id=? AND json_extract(event_json,'$.message.role')='toolResult' AND json_extract(event_json,'$.message.content') LIKE '%huntly_guard:final_submit_or_unproven_click%'").get(transcript.id).n:0
 database.close()
