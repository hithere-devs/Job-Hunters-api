...
function vncPort(i: number) { return 610 + i; }
function cdpPort(i: number) { return 920 + i; }
...
function stopProcess(i: number): Promise<boolean> {
  ...
  return new Promise(resolve => {
    ...
    setTimeout(() => { if (!done) { try { process.kill(pid, 'SIGKILL'); } catch {} finish(true); } }, 10_000);
  });
}
async function launch(i: number, mode: Exclude<Mode,'idle'>) {
  ...
  if (mode === 'apply') args.splice(-1, 0, `--remote-debugging-port=${cdpPort(i)}`, '--remote-debugging-address=127.0.0.1');
...
async function route(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (method==='POST' && path.endsWith('/apply')) { if (s.mode!=='idle') return json(res,409,{error:'busy',mode:s.mode}); const x=await launch(i,'apply'); return json(res,200,{mode:'apply',cdpUrl:`http://127.0.0.1:${cdpPort(i)}`,expiresAt:x?.expiresAt}); }
